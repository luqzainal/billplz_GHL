...
const BillplzConfig = () => {
  ...
  const handleVerifyPassword = async () => {
    setError(null);
    setLoadingStates(prev => ({ ...prev, production: true })); // Use production loading state for verification
    try {
      const response = await axios.post('/api/billplz/verify-password', { password: productionPassword });
      if (response.data.success) {
  ...
  const renderForm = (mode) => {
    ...
    return (
      <form className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1 flex rounded-md shadow-sm">
                <input
                  ...
                  disabled={isLoading || isProductionUnlocked} // Disable if loading or already unlocked
                  required
                />
                <button
                  type="button"
                  onClick={handleVerifyPassword}
                  disabled={isLoading || isProductionUnlocked || !productionPassword}
                  className={`inline-flex items-center px-3 py-2 border border-l-0 border-gray-300 rounded-r-md bg-gray-50 text-sm font-medium ${isProductionUnlocked ? 'text-green-600' : 'text-gray-700'} hover:bg-gray-100 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                  {isLoading ? (
        ...
        {/* --- Fields for Sandbox (always visible) --- */}      
        {mode === 'sandbox' && (
          <>
            <div>
              <label className="block text-sm font-medium text-gray-700">
        ...
        {/* --- Animated container for Production credential fields (only shows after unlock) --- */}      
        {mode === 'production' && (
           <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isProductionUnlocked ? 'max-h-[500px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
            {/* Show credential fields ONLY if production is unlocked */}      
            {isProductionUnlocked && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
